...
export async function fetchWorkflowLogs(): Promise<WorkflowLog[]> {
  const json = await requestJson<{ data: WorkflowLog[] }>("/api/logs");
  if (!Array.isArray(json.data)) {
    throw new Error(
      "fetchWorkflowLogs: invalid response shape (expected { data: WorkflowLog[] })",
    );
  }
  return json.data;
}
...
export async function fetchDailyReport(): Promise<DailyReport | null> {
  const json = await requestJson<{ data: DailyReport[] }>("/api/report");
  if (!Array.isArray(json.data)) {
    throw new Error(
      "fetchDailyReport: invalid response shape (expected { data: DailyReport[] })",
    );
  }
