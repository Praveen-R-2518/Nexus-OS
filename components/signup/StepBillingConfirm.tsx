...
export default function StepBillingConfirm({ snapshot }: StepBillingConfirmProps) {
  ...
  async function activatePlan() {
    if (!snapshot.workspaceId) {
      setError("Workspace not found. Go back and complete account setup.");
      return;
    }
    setBusy(true);
    setError("");
    ...
    if (snapshot.subscriptionId) {
      const { error: updErr } = await query.update(payload).eq("id", snapshot.subscriptionId);
      updateError = updErr;
    } else {
      const { error: insErr } = await query.insert({
